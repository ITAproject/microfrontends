...
const Products = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch(process.env.REACT_APP_WEB_API_GATEWAY_URL + '/products');
                const data = await response.json();
                const productsWithDiscounts = await Promise.all(data.map(async product => {
                    const discountResponse = await fetch(process.env.REACT_APP_WEB_API_GATEWAY_URL + `/discounts/${product.id}`);
    ...
    const deleteProduct = async (productId) => {
        try {
            await fetch(process.env.REACT_APP_WEB_API_GATEWAY_URL + `/products/${productId}`, {
                method: 'DELETE',
            });
